...
BEGIN
  FOR o IN (
    SELECT 'PROCEDURE' AS otype, 'STAMP_TENANT_CONTEXT' AS oname FROM dual UNION ALL
    SELECT 'VIEW',     'V_MY_ATTACHMENTS'  FROM dual UNION ALL
    ...
    SELECT 'VIEW',     'V_MY_CATEGORIES'   FROM dual UNION ALL
    SELECT 'VIEW',     'V_MY_PROJECTS'     FROM dual UNION ALL
    SELECT 'TABLE',    'ADMIN_AUDIT_LOG'   FROM dual UNION ALL
    ...
    BEGIN
      CASE o.otype
        ...
        WHEN 'PROCEDURE' THEN EXECUTE IMMEDIATE 'DROP PROCEDURE ' || o.oname;
      END CASE;
    EXCEPTION
      WHEN OTHERS THEN
