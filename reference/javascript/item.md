<!-- Source: https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/item.html -->
<!-- Interfaces: item -->
...
The item interface is used to access methods and properties of an Oracle APEX item including item plug-ins. You get access to the item interface for a page or column item with the [apex.item](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/apex.html#.fn:item) function.

An item interface can apply to either a page item or column item. Page items are items on the page backed by session state in any region. Column items are created by region types such as Interactive Grid that support editable columns. The state of a column item, including its value, changes according to the editing context (active record) of the region and is typically backed by data in an Oracle APEX [model](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/model.html).

Plug-in developers can define the behavior of their item by calling [apex.item.create](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/apex.item.html#.create).
...
A shorthand for getting and setting page item values. This autogenerated property is based on [item#getValue](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/item.html#getValue) and [item#setValue](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/item.html#setValue).

Note that you can pass other parameters using [item#setValue](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/item.html#setValue) such as `pDisplayValue` and `pSuppressChange`, which is not possible via `.value`.
...
Disables the Oracle APEX item, making it unavailable for editing. Not all items support being disabled. This only applies to items that can be edited. See also [item#enable](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/item.html#enable).
...
Enables the Oracle APEX item value that has been disabled, making it available for editing. Not all items support being disabled. This only applies to items that can be edited. See also [item#disable](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/item.html#disable).
...
Valid values for `type` are "separated" or "json-array". See `storageType` in [apex.item.create](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/apex.item.html#.create) for more information.

The property `separator` is only available when `type` is "separated". See `separator` in [apex.item.create](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/apex.item.html#.create) for more information.
...
See the example for [item#getValidity](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/item.html#getValidity) for an example of this function.
...
Returns the current value of an Oracle APEX item. The initial value of a page item comes from session state when the server renders the page. The initial value of a column item comes from the corresponding field value of the active record of the Oracle APEX [model](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/model.html). This function always returns the current value of the item, which may have been changed by the user or with the [item#setValue](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/item.html#setValue) method since it was initialized.

There are two shorthand functions related to `getValue`. The [\$v](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/global.html#$v) function that returns an item's value as a string. This will either be a single value, or if the item supports multiple values, will be a ':' colon separated list of values. Note this does not make use of the configurable separator or storage type for multivalued items. The [\$v2](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/global.html#$v2) function, which is just a shortcut to getValue and returns either a single value, or an array of values. See also [item#setValue](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/item.html#setValue).
...
See also [item#show](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/item.html#show).
...
See also [item#whenReady](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/item.html#whenReady) and the `delayLoading` property of [apex.item.create](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/apex.item.html#.create).
...
See also [item#hide](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/item.html#hide).
...
See also [item#isReady](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/item.html#isReady) and the `delayLoading` property of [apex.item.create](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/apex.item.html#.create).
