<!-- Source: https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/apex.region.html -->
...
## [apex](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/apex.html).region

The apex.region namespace contains global functions related to Oracle APEX regions. The [apex.region](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/apex.html#.fn:region) function provides access to a [region](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/region.html) interface for a specific region.
...
If the region is implemented with a jQuery UI style widget (using widget factory) then it should provide an implementation for the [region#widget](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/region.html#widget) method and define the [region#widgetName](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/region.html#widgetName) property so that the [region#call](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/region.html#call) method works. Note: jQuery UI is deprecated but the `call` method and `widgetName` property remain for backward compatibility.
...
<td class="description last">Region id or region static id. This comes from the PL/SQL plug-in parameter <code class="prettyprint">p_region.static_id</code>.</td>
</tr>
<tr>
...
#### (static) findClosest(pTarget) → {[region](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/region.html)\|null}

Returns the region that contains the `pTarget` element. Returns null if there is no `pTarget` element or if it is not in a region that has been initialized with a call to [apex.region.create](https://docs.oracle.com/en/database/oracle/apex/24.2/aexjs/apex.region.html#.create).
