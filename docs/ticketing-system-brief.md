...
- FR-6: System Admin can create/edit/deactivate users and assign each a role + company (agent creation additionally requires tier + `AGENT_PROJECTS`; provider-company users are **auto-granted CLIENT_USER**, decision Q). Client Admin can manage **Restricted-project invitations** (`USER_PROJECTS`) within their company. *(MUST)*
