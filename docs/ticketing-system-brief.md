...
- **Auto-escalation** on SLA breach — two-stage (warning at 75%, reassign at 90%), per-severity threshold via `SLA_TARGETS.escalation_pct` on the policy's target rows, functional + hierarchical escalation, workload-based agent selection, 5-min scheduler interval (FR-35)
- **Named SLA policies** with breach highlighting — `SLA_POLICIES` (Gold/Standard/Bronze/Internal) carrying per-severity `SLA_TARGETS`; projects are assigned a policy via `PROJECTS.sla_policy_id` (`is_default` fallback), colour-coded indicators (FR-23, decision S)
...
- **SLA target change audit trail** — log every change to `SLA_POLICIES`/`SLA_TARGETS` for ISO 20000 §8.6.3 compliance. *(The documentation fields `effective_from`, `approved_by`, `notes` landed in v1 on `SLA_POLICIES` — decision S; `effective_to` and the audit trail remain future.)*
...
### SLA & aging
- FR-23: **Named SLA policies, per severity** with declarative breach highlighting (computed at query time). `SLA_POLICIES` (decision S — industry pattern: Zendesk SLA policies, Autotask/Halo SLA templates, ServiceNow SLA definitions) are named tiers (e.g. Gold/Standard/Bronze/Internal) carrying documentation fields (`effective_from`, `approved_by`, `notes` — ISO 20000 §8.6.3); each policy's per-severity `SLA_TARGETS` rows map `severity` to `response_hours` + `resolution_days` + `escalation_pct`. A project is **assigned** a policy (`PROJECTS.sla_policy_id`); the `is_default` policy covers projects with no assignment. On ticket creation, `sla_due_date` is stamped (`created_at + resolution_days` from the governing policy's target for the ticket's severity). The ticket list shows a colour-coded breach indicator (🟢 On track / 🟡 At risk / 🔴 Breached). Admin-managed (System Admin manages policies + assignment). *(SHOULD)*
- FR-35: **Auto-escalation on SLA breach.** A scheduled job (`DBMS_SCHEDULER`, **runs every 5 minutes**) checks open tickets against their `sla_due_date`. Two-stage escalation with per-severity configurable thresholds (stored on the governing policy's `SLA_TARGETS.escalation_pct` rows, default 80% — decision S). **Stage 1 — Warning** (75% SLA elapsed): notify assigned agent + System Admin, log `SLA_WARNING`. **Stage 2 — Auto-reassign** (90% SLA elapsed): functional escalation to next-tier agent (tier read from `AGENT_PROJECTS` for the ticket's project — decision M revised) by lowest open-ticket count (FR-33 alignment), notify System Admin + ticket requester (ITIL: tell customer before they chase), log `ESCALATION`. **Hierarchical fallback:** if no higher tier exists (L4 / none mapped in the project), notify System Admin + Client Admin, flag `ESCALATION_BLOCKED`, keep current assignee (ITIL 4 + ISO 20000-1 §8.6.3 require a defined path — no dead ends). System-driven (distinct from manual reassignment in FR-26). *(SHOULD)*
...
| **PROJECTS** | Service engagements per client company (decision O) — e.g. "IT Support", "HR Systems Support" | `project_id` (PK), `company_id` (FK), `project_name`, `project_key` (VARCHAR2(10), **globally unique + company-prefixed** — e.g. "ACME-IT", "GLBX-IT"; kills duplicate-"IT Support" wrong-company errors, decision S), **`sla_policy_id`** (FK, nullable — the project's named SLA policy; NULL = the `is_default` policy applies, decision S), `description`, **`visibility`** (`OPEN`/`RESTRICTED`, default `OPEN` — decision Q: Open = whole company sees it; Restricted = invitation-only via `USER_PROJECTS`), `is_active` (Y/N), `created_at` |
...
erDiagram
    ...
    SLA_POLICIES ||--o{ SLA_TARGETS : "has per-severity targets"
    SLA_POLICIES ||--o{ PROJECTS : "governs"
...
- **O.** ✅ **Decided (2026-07-03): service engagements are modeled as projects.** New `PROJECTS` table between `COMPANIES` and `TICKETS`. `AGENT_COMPANIES` renamed to `AGENT_PROJECTS`. `SLA_TARGETS` rekeyed to `(project_id, severity)` *(superseded by decision S: now keyed to `(sla_policy_id, severity)`)*. `USER_PROJECTS` added for client-side project access control. See §2 and §5.
...
- **S.** ✅ **Decided (2026-07-03, landed in brief 2026-07-04 at design lock): named SLA policies (validated against Zendesk/ServiceNow/Autotask/Halo/Freshservice/ConnectWise patterns).** Per-project `SLA_TARGETS` rows become **`SLA_POLICIES`** — named tiers (Gold/Standard/Bronze/Internal) each carrying per-severity `SLA_TARGETS` (`response_hours`, `resolution_days`, `escalation_pct`) plus documentation fields (`effective_from`, `approved_by`, `notes`). Projects are **assigned** a policy via `PROJECTS.sla_policy_id`; the single `is_default` policy covers unassigned projects. Also: **project keys are globally unique + company-prefixed** (ACME-IT, GLBX-IT — kills duplicate-"IT Support" wrong-company errors), and admin editing is **one-door** — Project Detail tabs edit team/SLA/categories; global admin pages (SLA Policies, Agent-Project Mapping) are read-only rollups with Manage deep-links. Implemented in the mockup 2026-07-03. See §5 and FR-23/FR-35.
